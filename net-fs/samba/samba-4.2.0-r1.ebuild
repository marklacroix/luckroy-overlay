...
IUSE="acl addns ads aio avahi client cluster cups dmapi fam iprint
...
PATCHES=(
    "${FILESDIR}/${PN}-4.2.0-kdc_init.patch"
)
...
pkg_postinst() {
	...
	ewarn ""
	ewarn "Starting with Samba 4.2, the 4.x winbind implementation has been"
	ewarn "replaced by the updated 3.x winbind. Refer to the official Samba"
	ewarn "release notes for more details. Additionally, you *must* update your"
	ewarn "config to no longer use the %ACCOUNTNAME% and %WORKGROUP% variables."
	ewarn "These were changed to the 3.x style %U and %D variables in the 4.2"
	ewarn "branch in the fix for https://bugzilla.samba.org/show_bug.cgi?id=10324"
